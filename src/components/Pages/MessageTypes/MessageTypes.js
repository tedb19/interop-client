...
import { saveStat, messageTypesSecondaryMenuData, getMessageTypeObj, entitiesSubscribedTo, addMessageType } from '../../../utils/data.utils'
...
export class MessageTypes extends Component {
    ...
    async componentDidMount() {
        ...
        const [ messageType, entities ] = await Promise.all(
            [getMessageTypeObj(currentPage), entitiesSubscribedTo(currentPage)]
        )
        this.setState({ 
    ...
    handleMessageTypeMenuItemClick = async (e, { name }) => {
        const [ messageType, entities ] = await Promise.all(
            [getMessageTypeObj(name), entitiesSubscribedTo(name)]
        )
        this.setState({
            messageType, entities,
    ...
    jsonContent = () => {
        let json = ''
        if(!this.state.ActiveMessageTypeMenuItem) return <Container>Message sample has not been uploaded</Container>
        const imgName = this.state.ActiveMessageTypeMenuItem.includes('Adx') ? `${this.state.ActiveMessageTypeMenuItem.replace(/ /g, '-')}.xml` : `${this.state.ActiveMessageTypeMenuItem.replace(/ /g, '-')}.json`
        try{
            let imgPath = require(`../../../images/SampleMessages/${imgName}`)
            json = imgName.includes('.xml') ? '' :
            <ReactJson
                src={imgPath}
                theme="monokai"
                name="message"
    ...
    formModal = () => {
        const submitHandler = (!this.state.name || !this.state.verboseName || !this.state.description) ? this.handleEmptySubmit : this.handleSubmit
    ...
    messages = () => {
        const newMessageTypeDetails = {
            ...
            content: `${this.state.verboseName} message type has been added successfully! Systems can now subscribe to this message type`
        }
