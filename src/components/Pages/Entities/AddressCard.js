...
export const AddressCard = (props) => {
    const protocolDescriptions = {
        http: 'This should be a POST endpoint on the participating system. The IL will be doing an HTTP POST to this address.',
        tcp: 'This should be a TCP port where the participating system will be listening on. IL will be sending the messages to this port.'
    }
    ...
    const addressAvailable = [
        {
          ...
          description: props.addressType === 'HTTP' ? protocolDescriptions.http : protocolDescriptions.tcp,
    ...
    return (
        <Card.Group className="address-card-group" items={items}  />
    )
}
