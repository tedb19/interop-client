...
export class MessageSubscription extends Component {
  ...
  newSubscriptionDialogOnClose = () =>
    this.setState({ newSubscriptionDialogOpen: false });
  duplicateSubscriptionDialogOnClose = () =>
    this.setState({ duplicateSubscriptionDialogOpen: false });
  unsubscribeDialogOnClose = () =>
    this.setState({ unsubscribeDialogOpen: false });
  ...
  subscribersLabels = () => {
    const labels = this.state.subscriptions ? (
      this.state.subscriptions.map(subscriber => {
        return (
              ...
              entitiesSubscribedTo(messageTypeName)
                .then(entities => {
                  const entity = entities.find(
                    entity => entity.name === data.subscribers
                  );
                  entity
                    ? this.setState({
  ...
  render() {
    ...
    const subscriptionMessages = {
      dialogConfirmationMsg: (
        <p>
          Subscribe <strong>{this.state.currentEntity}</strong> to the{" "}
          <strong>{this.state.currentMessageType.replace(/_/g, " ")}</strong>{" "}
          message type?
        </p>
      ),
      messageContent: `${
        this.state.currentEntity
      } has been successfully subscribed to the ${
    ...
    const duplicateSubscriptionMessages = {
      dialogConfirmationMsg: (
        <p>
          <strong>{this.state.currentEntity}</strong> is already subscribed to
          the{" "}
          <strong>{this.state.currentMessageType.replace(/_/g, " ")}</strong>{" "}
          message type!
        </p>
    ...
    const unsubscriptionMessages = {
      dialogConfirmationMsg: (
        <p>
          Unsubscribe <strong>{this.state.currentEntity}</strong> from the{" "}
          <strong>{this.state.currentMessageType.replace(/_/g, " ")}</strong>{" "}
          message type?
        </p>
      ),
      messageContent: `${
        this.state.currentEntity
      } has been successfully unsubscribed from the ${
        this.state.currentMessageType
      } message type.`,
            ...
            <Grid.Column width={16}>
              ...
              <Segment inverted className="segment-subscriptions">
                ...
                <Label className="stats-sub-header-right">
                  Systems subscribed to the msg type
                </Label>
                ...
                <DialogModal
                  ...
                  isClicked={
                    this.handleduplicateSubscriptionDialogButtonClicked
                  }
              ...
              <Header as="h4" className="stats-header">
                Drag and drop a system to a message type to initiate a
                subscription
              </Header>
              {this.state.entities ? (
                <SubscriberPool entities={this.state.entities} />
              ) : null}
            </Grid.Column>
          </Grid>
        </MainContent>
